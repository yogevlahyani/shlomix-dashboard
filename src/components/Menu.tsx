...
class Menu extends React.Component<PropsTypes, StateTypes> {
  ...
  fetchItems: any = (): void => {
    axios.get('http://104.236.92.123/menu/byItem/' + this.state.item._id)
      .then((res: any): void => {
        console.log(res);
        this.setState({
          menuItems: res.data
  ...
  submitItem: any = (): void => {
    ...
    if(name !== '' && description !== '' && price > 0) {
      axios.post('http://104.236.92.123/menu/addMenu', {
        itemId: item._id,
        name,
        description,
        price
  ...
  _delItem = (menu: any): void => {
    ...
    .then((willDelete) => {
      if (willDelete) {
        axios.post('http://104.236.92.123/menu/killItem', {
          menuId: menu._id
        })
        .then((res: any): void => {
          swal(res.data.feedback, {
  ...
  updateItemName: any = (menu: any): void => {
    swal({
      ...
      if(e != null && e != '') {
        axios.post('http://104.236.92.123/menu/updateItem', {
          id: menu._id,
          name: e
        }).then((res: any): void => {
          console.log(res);
